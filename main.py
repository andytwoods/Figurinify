...
def _extract_model_id(text: str) -> Optional[str]:
    """
    best-effort: pulls a model id like:
...
def resolve_to_glb_url(user_input: str, log) -> Resolved:
    ...
    3) model page built from a detected model id, then scan html for .glb
    """
    s = user_input.strip()
    ...
    # 2) user pasted a normal url (maybe a model page)
    if _is_url(s):
    ...
    if model_id:
        # mesh model page pattern (best-effort)
        # if your real links differ, adjust this template
        page = base64.b64decode("aHR0cHM6Ly93d3cubWVzaHkuYWkvM2QtbW9kZWxzLw==").decode()
    ...
    raise ValueError(
        ...
        "tip – use the download/export option and copy the direct .glb link if available."
    )
...
class TkDownloaderApp(tk.Tk):
    def __init__(self) -> None:
        ...
        lbl = ttk.Label(self, text="paste a share link, model id, or a direct .glb url:")
    ...
    def on_download(self) -> None:
        ...
        def worker():
            try:
                ...
                self.log("download complete – open Figurinify and use the site’s file picker to load your model.")
